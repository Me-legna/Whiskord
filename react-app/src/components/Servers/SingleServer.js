...
// import serverDetails thunk and channels thunk
...
export default function SingleServer() {
    ...
    const serverName = useSelector((state) => state.singleServer.name)
        ...
        <div className='server-main-page-div'>
            ...
            <div className='server-body'>
                <div className="sever-list-and-dm-div">
                    ...
                    <div className='single-server-button' key={serverId} onClick={() => history.push(`/servers/${serverId}`)}>{serverName}</div>
                </div>
                <div className='channels-list'>{serverChannelsArr.map(({ id }) => (
                    <div className='single-channel' onClick={() => history.push(`/channels/${channelId}`)}>{channelName}</div>
                ))}
                </div>
            ...
            <div className='server-feed-div'>
                <div className='server-feed-block'> {channelMessagesArr.map(({ user, created_at, message })=> (
            ...
            <div className='members-list'>{membersArr.map(({ username }) => (
                     <div className='button'>{<OpenModalButton
